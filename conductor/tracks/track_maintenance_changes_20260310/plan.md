...
- [x] Task: Audit `inventory.asset.maintenance` and `inventory.asset.maintenance.line` models to identify where to add `tracking=True`.
- [x] Task: Identify the exact Odoo 18 view structure for the maintenance history list on the Asset form.
- [x] Task: Conductor - User Manual Verification 'Phase 1: Research and TDD Setup' (Protocol in workflow.md)

## Phase 2: Backend Implementation - Field Tracking
- [x] Task: Write a test case that creates a maintenance record in the 'done' state and verifies that changing notes/images does NOT log a message in the chatter yet.
