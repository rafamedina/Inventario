...
- [ ] Task: Manually audit all `__manifest__.py` and `__init__.py` files for broken paths and imports.
    - [ ] List all data, demo, and assets files mentioned in `__manifest__.py` and verify their existence at the specified paths.
    - [ ] Trace all imports in `__init__.py` files across the module to identify missing or incorrectly referenced modules.
- [ ] Task: Create a reproduction script or unit test that clearly fails due to the project structure change.
    - [ ] Create a script that attempts to import the main module and check its manifest structure.
    - [ ] Run the reproduction and confirm failure.
