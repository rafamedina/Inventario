...
class TestEmployeeAssetLabelUI(common.TransactionCase):
    def test_employee_form_contains_asset_smart_button(self):
        ...
        self.assertIn(
            'name="action_view_employee_assets"',
            arch,
            "The smart button should call 'action_view_employee_assets'",
        )
        self.assertIn(
            'class="oe_stat_button"',
            arch,
            "The button should have the 'oe_stat_button' class",
        )
        self.assertIn('icon="fa-barcode"', arch, "The button should have the 'fa-barcode' icon")
        self.assertIn(
            'field name="asset_count"',
            arch,
            "The button should display the asset_count field",
        )
    ...
    def test_asset_count_computation(self):
        ...
        # Create assets
        self.env["inventory.asset"].create(
            {
                "nombre": "Asset 1",
