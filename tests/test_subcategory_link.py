    ...
    def test_subcategory_requires_category(self):
        ...
        with self.assertRaises(ValidationError):
            with self.cr.savepoint():
                self.Subcategory.create({"nombre": "Test Sub", "codigo": "TSUB", "categoria_id": False})
                self.Subcategory.flush_model()

    def test_dynamic_filtering_domain(self):
        """Test that subcategories are filtered by category in Assets"""
        sub_nvg = self.Subcategory.create({"nombre": "Navegador", "codigo": "NVG", "categoria_id": self.cat_apps.id})
        sub_srv = self.Subcategory.create({"nombre": "Servidor", "codigo": "SRV", "categoria_id": self.cat_hw.id})

        asset = self.env["inventory.asset"].new({"categoria_id": self.cat_apps.id})
        ...
        available_subs = self.Subcategory.search([("categoria_id", "=", asset.categoria_id.id)])
