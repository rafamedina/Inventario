...
class TestInventoryAssetImport(common.TransactionCase):
    ...
    def test_plans_asset_multi_create_unique_id(self):
        ...
        subcat = self.env['inventory.subcategory'].create({
            'nombre': 'Subcat 1', 
            'codigo': 'SC1',
            'categoria_id': self.cat.id
        })
        vals_list = [
            {'nombre': 'Plan 1', 'subcategoria_id': subcat.id, 'categoria_id': self.cat.id},
            {'nombre': 'Plan 2', 'subcategoria_id': subcat.id, 'categoria_id': self.cat.id},
        ]
    ...
    def test_plans_alert_compute_name(self):
        ...
        plan = self.env['plans.asset'].create({
            'nombre': 'Test Plan',
            'categoria_id': self.cat.id,
            'subcategoria_id': self.env['inventory.subcategory'].create({
                'nombre': 'Sub', 
                'codigo': 'S',
                'categoria_id': self.cat.id
            }).id
