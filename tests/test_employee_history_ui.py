...
class TestEmployeeHistoryUI(common.TransactionCase):
    def setUp(self):
        ...
        self.subcategory = self.env["inventory.subcategory"].create({"nombre": "SUB", "codigo": "SUB", "categoria_id": self.category.id})
    ...
    def test_history_shows_asset_identifier(self):
        ...
        history = self.env["inventory.asset.history"].search([("asset_id", "=", self.asset.id)], limit=1)
