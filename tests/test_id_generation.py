...
class TestIDGeneration(common.TransactionCase):
    def setUp(self):
        ...
        self.subcat = self.env["inventory.subcategory"].create({"nombre": "Subcat 1", "codigo": "SC1", "categoria_id": self.cat.id})
    ...
    def test_plans_asset_sequence(self):
        ...
        self.assertTrue(
            plan1.identificador.startswith("PLAN-"),
            f"Expected prefix PLAN-, got {plan1.identificador}",
        )
        self.assertTrue(
            plan2.identificador.startswith("PLAN-"),
            f"Expected prefix PLAN-, got {plan2.identificador}",
        )
    ...
    def test_inventory_asset_sequence(self):
        ...
        self.assertTrue(
            asset1.uuid_activo.isdigit(),
            f"Expected numeric ID, got {asset1.uuid_activo}",
        )
        self.assertTrue(
            asset2.uuid_activo.isdigit(),
            f"Expected numeric ID, got {asset2.uuid_activo}",
        )
    ...
    def test_manual_id_preservation(self):
        ...
        asset = self.env["inventory.asset"].create({"nombre": "Manual Asset", "uuid_activo": "MANUAL-001"})
    ...
    def test_identificador_final_computation(self):
        ...
        expected_end = ".2026"
        self.assertTrue(
            asset.identificador_final.startswith(expected_start),
            f"Expected prefix {expected_start}, got {asset.identificador_final}",
        )
        self.assertTrue(
            asset.identificador_final.endswith(expected_end),
            f"Expected suffix {expected_end}, got {asset.identificador_final}",
        )
