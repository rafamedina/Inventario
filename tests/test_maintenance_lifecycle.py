...
class TestMaintenanceLifecycle(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.tech = self.Employee.create({
            'name': 'John Tech',
    ...
    def test_maintenance_alert_generation(self):
        ...
        self.assertTrue(new_activities, "An alert activity should have been created for maintenance in 1 week")
    ...
    def test_maintenance_done_flow(self):
        ...
        self.assertEqual(self.asset.proximo_mantenimiento, expected_next, "Next maintenance date should be in 30 days")
        
        # Verify message post
        messages = self.asset.message_ids.mapped('body')
        self.assertTrue(any("Tarea completada" in m for m in messages), "A message should be posted when maintenance is done")
