...
class TestAssetTraceability(common.TransactionCase):
    ...
    def test_auto_history_logging(self):
        ...
        history = self.env["inventory.asset.history"].search(
            [("asset_id", "=", self.asset.id), ("role", "=", "responsible")],
            order="id desc",
            limit=1,
        )  # Get the latest one
        ...
        self.assertTrue(
            history.exists(),
            "Automatic history record should be created on responsible change",
        )
    ...
    def test_employee_inheritance(self):
        ...
        self.assertIn(
            self.asset,
            self.employee_1.current_responsible_asset_ids,
            "Asset should be in current responsible list",
        )
        ...
        self.assertIn(
            self.asset,
            self.employee_1.historical_asset_ids,
            "Asset should be in historical list for Employee 1",
        )
